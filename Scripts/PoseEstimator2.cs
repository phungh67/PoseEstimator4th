...
public class PoseEstimator2 : MonoBehaviour
{
    ...
    private Utils2.Keypoint[][] poses;

    // Array of pose skeletons
    private PoseSkeleton2[] skeletons;
    ...
    void Update()
    {
        ...
        {
            preProcessFunction = Utils2.PreprocessMobileNet;
        }
        else
        {
            preProcessFunction = Utils2.PreprocessResNet;
        }
        ...
        {
            foreach (PoseSkeleton2 skeleton in skeletons)
            {
                skeleton.Cleanup();
            }
        ...
        for (int i = 0; i < skeletons.Length; i++)
        {
            if (i <= poses.Length - 1)
            {
                ...
                skeletons[i].UpdateKeyPointPositions(poses[i], scale, videoTexture, false, minConfidence);
    ...
    private void InitializeVideoScreen(int width, int height)
    {
        ...
        videoScreen.localScale = new Vector3(width / 2, height / 2, videoScreen.localScale.z);
        // Adjust the VideoScreen position for the new videoTexture
        videoScreen.position = new Vector3(width, height, 1);
    }
    ...
    private void InitializeCamera()
    {
        ...
        GameObject mainCamera = GameObject.Find("Camera 2");
        // Adjust the camera position to account for updates to the VideoScreen
        mainCamera.transform.position = new Vector3(videoDims.x, videoDims.y, 0f);
    ...
    private void ProcessOutput(IWorker engine)
    {
        ...
        if (estimationType == EstimationType.SinglePose)
        {
            ...
            poses = new Utils2.Keypoint[1][];

            // Determine the key point locations
            poses[0] = Utils2.DecodeSinglePose(heatmaps, offsets, stride);
        }
        else
        {
            // Determine the key point locations
            poses = Utils2.DecodeMultiplePoses(
                heatmaps, offsets,
    ...
    private void InitializeSkeletons()
    {
        ...
        skeletons = new PoseSkeleton2[maxPoses];

        // Populate the list of pose skeletons
        for (int i = 0; i < maxPoses; i++) skeletons[i] = new PoseSkeleton2(pointScale, lineWidth);
    }
}
